...
public class Cardm23 extends Card {
    ...
    public void play(PlayerModel player, CardDeckModel deck, RoomListModel rlm) {
        if(player.getCurrentRoom().getName().equals("Computer Lab") && player.getIntegrity() >= 4) {
            player.addQuality(3);
            //player.addCraft(1);
            //System.out.println(player.getPlayer().getName() + " played " + getName() + " for 3 Quality Points and 1 Craft Chip");

            //player.addIntegrity(1);
            //System.out.println(player.getPlayer().getName() + " played " + getName() + " for 3 Quality Points and 1 Integrity Chip");

            //player.addLearning(1);
            //System.out.println(player.getPlayer().getName() + " played " + getName() + " for 3 Quality Points and 1 Learning Chip");
        }
        else {
            int discard = (int) Math.random() * (player.getHand().size() - 1);
            deck.discard(player.getHand().remove(discard));
            System.out.println(player.getPlayer().getName() + " played " + getName() + " and discarded 1 Game Card");
        }
    }
}