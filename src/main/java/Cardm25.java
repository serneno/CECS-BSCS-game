...
public class Cardm25 extends Card {
    ...
    public void play(PlayerModel player, CardDeckModel deck, RoomListModel rlm) {
        if(player.getCurrentRoom().getName().equals("Lactation Lounge") && player.getLearning() >= 2) {
            player.addQuality(5);
            //player.addCraft(1);
            //System.out.println(player.getPlayer().getName() + " played " + getName() + " for 5 Quality Points and 1 Craft Chip");
            ...
            //System.out.println(player.getPlayer().getName() + " played " + getName() + " for 5 Quality Points and 1 Learning Chip");
        }
        else {
